...
> 在 Popup 主视图中提供英文长难句的 AI 结构化分析，帮助学习者理解复杂句式的句法结构、语法要点和核心含义。
...
```
用户输入句子 → Popup 发送 ANALYZE_SENTENCE 消息
...
与页面级功能（翻译/释义/混杂中英）不同，长难句分析是**纯 Popup 交互**——不依赖 Content Script，不操作页面 DOM。
...
| **消息协议** | `types/messages.ts` | 新增 `ANALYZE_SENTENCE` 消息类型和响应接口 |
...
| **前端交互** | `popup/popup.html` | 输入区域 + 结果卡片 HTML 结构 |
| **前端逻辑** | `popup/popup.ts` | 事件绑定、分析触发、结果渲染 |
| **视觉样式** | `popup/popup.css` | 彩色标签、从句卡片、短语列表等样式 |
