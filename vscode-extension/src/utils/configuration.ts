...
export class ConfigurationManager {
	...
	private static validatePromptTemplates(
		templates: any,
		defaultTemplates?: { system?: string, user?: string }
	): { system: string, user: string } | undefined {
		if (!templates || typeof templates !== 'object') {
			return defaultTemplates ? {
				system: defaultTemplates.system || DEFAULT_PROMPT_TEMPLATES.system,
				user: defaultTemplates.user || DEFAULT_PROMPT_TEMPLATES.user
			} : undefined;
		}
		...
		return {
			system: system || defaultTemplates?.system || DEFAULT_PROMPT_TEMPLATES.system,
			user: user || defaultTemplates?.user || DEFAULT_PROMPT_TEMPLATES.user
		};
	}
	...
	static getConfig(): ExtensionConfig {
		...
		const validatedPromptTemplates = this.validatePromptTemplates(globalPromptTemplates, DEFAULT_PROMPT_TEMPLATES)!;
	...
	static async updateConfig(
		...
		try {
			...
			if (updates.providers) {
				throw new Error(
					'不能直接更新providers对象，请使用updateProviderConfig方法更新特定提供商配置'
				);
			}
	 ...
	 * @returns 扁平化的配置键值对
	 */
	private static flattenConfigUpdates(updates: Partial<ExtensionConfig>): Record<string, any> {
