...
export class AnalysisPanel {
	private readonly viewType = 'lingurideAnalysisView';
	 ...
	 * 更新侧边栏Webview视图
	 */
	private updateWebviewView(webviewView: vscode.WebviewView): void {
		if (!this.currentResult) {
			return;
		}
...
<body>
	 ...
	 * 显示详细视图
	 */
	private async showDetailedView(): Promise<void> {
		if (!this.currentResult) {
			return;
		}
	 ...
	 * 获取详细HTML内容
	 */
	private getDetailedHtml(): string {
		// 简化实现，实际项目中可以扩展
		return `<!DOCTYPE html>
<html>
<head>
