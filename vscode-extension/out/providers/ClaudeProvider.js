 ...
 */
class ClaudeProvider extends ILLMProvider_1.BaseProvider {
    constructor(config) {
        super(config);
        this.name = 'Claude';
        this.id = 'claude';
        this.description = '使用Anthropic Claude模型进行英文难度分析';
        // 创建Axios实例
    ...
    async analyze(options) {
        try {
            ...
            const systemPrompt = promptBuilder_1.PromptBuilder.getSystemPrompt(this.id, config);
            const userPrompt = promptBuilder_1.PromptBuilder.getUserPrompt(options.text, this.id, config, options.language);
            // 准备请求数据（Claude API格式）
            const requestData = {
